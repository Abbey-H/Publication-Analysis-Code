...
The following code outlines the Pre-acqusition and Acquisition phase analyses completed for Hammell et al., "Temporal Course of Over-Generalized Conditioned Threat Expectancies in Posttraumatic Stress Disorder". Note that this document is meant for reproducibility/replicability purposes for Pre-acqusition and Acquisition phases. To understand the general procedure for the SSANOVA analyses and contrast analyses, it is recommended that you review the "Hammell_Analysis_GEN.rmd" file, which contains a better walkthrough/outline of the procedure. 
...
dataPREACQ <- "data.csv" #give name of datafile
...
rmod <- bigssp(Risk ~ Group * Stimulus,
               ...
               type = types,
               random = ~ Group | SubID,
               skip.iter = F) #it's a simple analysis, so we don't see much of an increase in computational run-time with skip.iter = F
```
...
```{r}
datapath <- "~/Desktop/" #set directory of where data is located
dataACQ <- "data.csv" #give name of datafile
...
cor(ptsd.ACQ.df$Risk, rmod.ACQ$fitted.values)^2
cor(ptsd.ACQ.df$Risk, yhat)^2
```
...
### Cosine diagnostics
```{r}
diagnost <- summary(rmod.ACQ, diagnostics = TRUE)
```
